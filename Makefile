...
.PHONY: run/api
run/api:
	go run ./cmd/api -db-dsn=${QUOTABLE_DB_DSN}
...
# 	CREATE DATABASE IF NOT EXISTS quotable;
# 	\c greenlight
# 	CREATE ROLE quotable WITH LOGIN PASSWORD '${password}';
...
## audit: tidy depencies and format, vet and test all code
.PHONY: audit
audit:
...
git_desc = ${shell git describe --dirty --always --long --tags}
